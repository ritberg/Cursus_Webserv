...
{
	std::cout << std::endl << "----Awaiting connection [" << server_config["listen"] << "]----" << std::endl << std::endl;
...
void ServerSocket::Init(const std::string &configFile)
{
	...
	server_addr.sin_port = htons(std::stoi(server_config["listen"]));
